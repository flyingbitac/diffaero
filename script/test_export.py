...
class agent_for_deploy(nn.Module):
    def __init__(self,agent:World_Agent):
        ...
        self.register_buffer("hidden_state",torch.zeros(agent.hidden.shape[0],agent.state_model.cfg.hidden_dim))
...
def main(cfg: DictConfig):
    ...
    deploy_agent = agent_for_deploy(agent).to(device)
    
    test(cfg, deploy_agent, env, logger)
