...
def learn(
    ...
    for i in pbar:
        t1 = pbar._time()
        env.detach()
        cumulative_loss, score_loss, env_info = agent.step(env)
        ...
        pbar.set_postfix({
            ...
            "score_loss": f"{score_loss:.3f}",
            "env_loss": f"{cumulative_loss:.3f}",
            # "l_episode": f"{l_episode:.1f}",
            # "success_rate": f"{success_rate:.2f}",
            "fps": f"{int(cfg.env.n_envs/(pbar._time()-t1)):,d}"})
        log_info = {
            "env_loss": env_info["loss_components"],
            "agent_loss/cumulative_loss": cumulative_loss,
            "agent_loss/score_loss": score_loss,
...
def main(cfg: DictConfig):
    ...
    if hasattr(env, "update_sensor_data"):
        profiler.add_function(ObstacleAvoidance.update_sensor_data)
    ...
    agent = YOPO(cfg.algo, device)
    profiler.add_function(YOPO.step)
    profiler.add_function(YOPO.act)
