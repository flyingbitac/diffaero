...
def train_worldmodel(world_model:StateModel,replaybuffer:ReplayBuffer,opt,training_hyper,logger:Logger):
    for _ in range(training_hyper.worldmodel_update_freq):
        sample_state, sample_action, sample_reward, sample_termination,sample_reward_components = replaybuffer.sample(training_hyper.batch_size,training_hyper.batch_length)
