...
class PositionControl(BaseEnv):
    def __init__(self, cfg: DictConfig, device: torch.device):
        ...
        self.state_dim = 13
        self.renderer = None if cfg.render.headless else PositionControlRenderer(
            cfg.render, device
        )
    ...
    @timeit
    def get_state(self, with_grad: bool = False) -> Tensor:
        state = torch.cat(
            [
    ...
    @timeit
    def get_observations(self, with_grad: bool = False) -> Tensor:
        if self.obs_frame == "local":
    ...
    def loss_and_reward(
        ...
        if self.dynamic_type == "pointmass":
            ...
            loss_components = {
                "vel_loss": vel_loss.mean().item(),
                "jerk_loss": jerk_loss.mean().item(),
                "attitude_loss": attitude_loss.mean().item(),  # fixed key spelling
                "pos_loss": pos_loss.mean().item(),
                "total_loss": total_loss.mean().item(),
                "total_reward": total_reward.mean().item(),
            }
    ...
    def terminated(self) -> Tensor:
        ...
        out_of_bound = torch.any(self.p < -p_range, dim=-1) | torch.any(
            self.p > p_range, dim=-1
        )
        return out_of_bound

    def export_obs_fn(self, path: str) -> None:
...
class Sim2RealPositionControl(PositionControl):
    def __init__(self, cfg: DictConfig, device: torch.device):
        ...
        self.square_positions = torch.tensor(
            [
                [self.square_size, -self.square_size, 0.0],
                [-self.square_size, -self.square_size, 0.0],
                [-self.square_size, self.square_size, 0.0],
                [self.square_size, self.square_size, 0.0],
            ],
    ...
    def update_target(self) -> None:
        ...
        idx = torch.floor(t / self.switch_time).long() % self.square_positions.shape[0]
        self.target_pos = self.square_positions[idx]
    ...
    def terminated(self) -> Tensor:
        p_range = torch.full_like(self.L.value.unsqueeze(-1), self.square_size * 2.0)
        out_of_bound = torch.any(self.p < -p_range, dim=-1) | torch.any(
            self.p > p_range, dim=-1
        )
        return out_of_bound
