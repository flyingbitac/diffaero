...
class Critic(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.is_q_function = is_q_function
        state_dim = mlp_state_dim(state_dim)
        state_dim = state_dim + action_dim if self.is_q_function else state_dim
...
class StochasticActorCritic(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.critic = Critic(state_dim, hidden_dim, 1, is_q_function=False)
...
class RPLActorCritic(StochasticActorCritic):
    ...
    def rpl_obs(self, obs: TensorDict) -> Tensor:
        with torch.no_grad():
            anchor_action, _, _, _ = self.anchor_agent.get_action(
                obs["state"], test=True)
        return torch.cat([preprocess_state(obs), anchor_action], dim=-1)
    ...
    def get_action_and_value(self, obs, sample=None, test=False):
        ...
        with torch.no_grad():
            anchor_action, _, _, _ = self.anchor_agent.get_action(
                obs["state"], test=True)
        rpl_obs = torch.cat([preprocess_state(obs), anchor_action], dim=-1)
